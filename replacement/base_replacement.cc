...
uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    if (way == NUM_WAY) {
        for (way=0; way<NUM_WAY; way++) {
            //优先替换prefetch的块
            if(block[set][way].prefetch == 1&&block[set][way].lru>lru_temp){
                lru_temp=block[set][way].lru;
                way_temp=way;
            }
        }
         
           
        if(way_temp!=-1&&/*block[set][way_temp].reliable<30*/lru_temp>NUM_WAY/2){   //预取的块必须要呆满一段时间才能替换 不能出现预取块频繁替换的情况
            block[set][way_temp].prefetch = 0;//下次不再替换
            //block[set][way_temp].reliable=0;
            return way_temp;
        }
        way=NUM_WAY;
    }
