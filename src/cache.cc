...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    block[set][way].reliable=packet->reliable;//for prefetch
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata,uint64_t reliable)
{
    pf_requested++;
    ...
    if (PQ.occupancy < PQ.SIZE) {
        if ((base_addr>>LOG2_PAGE_SIZE) == (pf_addr>>LOG2_PAGE_SIZE)) {
            ...
            PACKET pf_packet;
            pf_packet.reliable=reliable;//for prefetch
